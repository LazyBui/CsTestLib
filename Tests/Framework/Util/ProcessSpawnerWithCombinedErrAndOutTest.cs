...
namespace Test {
	...
	public class ProcessSpawnerWithCombinedErrAndOutTest {
		...
		public void SimpleInterspersed() {
			...
			Assert.DoesNotThrow(() => {
				process = new ProcessSpawnerWithCombinedErrAndOut(TestApplications.SimpleInterspersed);
				result = process.Run();
			});
...
aabbccddeeff");
		...
		public void BlankLinesInterspersed() {
			...
			Assert.DoesNotThrow(() => {
				process = new ProcessSpawnerWithCombinedErrAndOut(TestApplications.BlankLinesInterspersed);
				result = process.Run();
			});
			Assert.NotNull(process);
			Assert.NotNull(result);
...
abc");
		...
		public void NoOutput() {
			...
			Assert.DoesNotThrow(() => {
				process = new ProcessSpawnerWithCombinedErrAndOut(TestApplications.NoOutput);
				result = process.Run();
			});
			Assert.NotNull(process);
			Assert.NotNull(result);
